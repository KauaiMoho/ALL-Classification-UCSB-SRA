...
model_raw = efficientnet_b3(weights='IMAGENET1K_V1')
model_raw.aux_logits = False
model = nn.Sequential(
          model_raw,
          nn.Dropout(p=0.35)
        )
# model.load_state_dict(torch.load('feature_extractor_89'))
...
xgbm = xgb.XGBClassifier(eta = 0.1, eval_metric=eval_metric)
